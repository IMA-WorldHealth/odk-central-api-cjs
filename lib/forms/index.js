...
/**
 * @function assignActorToFormRole
 *
 * @description
 * Given a roleId, which may be a numeric ID or a string role system name, and
 * a numeric actorId, assigns that Role to that Actor for this particular Form.
 */
exports.assignActorToFormRole = async function assignActorToFormRole(projectId, xmlFormId, roleId, actorId) { // eslint-disable-line
  return (await client()).post(`projects/${projectId}/forms/${xmlFormId}/assignments/${roleId}/${actorId}`).json();
};
